...
def survey():
    ...
    # Convert the similarity matrix to a DataFrame
            ...
            # Create DataFrame for df_list with df_list as column named recommendation

            df_result = pd.DataFrame(df_list)

            df_result['Recommnendation'] = df_result['Top1 Product'].astype(str) + ',' + df_result['Similarity Score Top1'].astype(str) + ',' + df_result['Top2 Product'].astype(str) + ',' + df_result['Similarity Score Top2'].astype(str) + ',' + df_result['Top3 Product'].astype(str) + ',' + df_result['Similarity Score Top3'].astype(str) + ',' + df_result['Top4 Product'].astype(str) + ',' + df_result['Similarity Score Top4'].astype(str) + ',' + df_result['Top5 Product'].astype(str) + ',' + df_result['Similarity Score Top5'].astype(str)

            # Set the key as the index
            df_result.set_index("CustomerID", inplace=True)

            df_result.drop(columns=['Top1 Product','Similarity Score Top1','Top2 Product','Similarity Score Top2','Top3 Product','Similarity Score Top3','Top4 Product','Similarity Score Top4','Top5 Product','Similarity Score Top5'], inplace=True)
